...
public final class zzfoa {
    ...
    public static PendingIntent zza(Context context, int i10, Intent intent, int i11, int i12) {
        boolean z10 = true;
        zzfou.zzf((i11 & 95) == 0, "Cannot set any dangerous parts of intent to be mutable.");
        zzfou.zzf(intent.getComponent() != null, "Must set component on Intent.");
        if (zzb(0, 1)) {
            zzfou.zzf(!zzb(i11, 67108864), "Cannot set mutability flags if PendingIntent.FLAG_IMMUTABLE is set.");
        } else {
            if (Build.VERSION.SDK_INT >= 23 && !zzb(i11, 67108864)) {
                z10 = false;
            }
            zzfou.zzf(z10, "Must set PendingIntent.FLAG_IMMUTABLE for SDK >= 23 if no parts of intent are mutable.");
        }
